...
示例：
  ...
  http://127.0.0.1:8000/docs/ew-usage-guide-v1.json  — 同上内容 JSON
  POST /docs/ew-usage-guide-v1/email  — 登录用户填写邮箱，发送守则 PDF（需 EW_SMTP_*）
...
from function.api_config import (
    ew_smtp_configured,
    integration_snapshot,
    reload_api_env,
    save_order_google_miles_max_ui,
)
...
from function.send_usage_guide_email import send_usage_guide_pdf_email
from function.usage_guide_pdf import build_usage_guide_v1_pdf_bytes, usage_guide_v1_json_payload
...
def home(
    request: Request,
    usage_email: str | None = Query(None),
    usage_email_err: str | None = Query(None),
) -> HTMLResponse:
    ...
    return HTMLResponse(
        content=render_home_page(
            ...
            session_user=cap,
            role=role,
            usage_email_ok=usage_email == "ok",
            usage_email_err=usage_email_err,
            smtp_configured=ew_smtp_configured(),
        )
    )
...
@app.get("/docs/ew-usage-guide-v1.json", response_model=None)
def usage_guide_v1_json() -> JSONResponse:
    """第一版使用守则（JSON，与 PDF 同源）。"""
    return JSONResponse(content=usage_guide_v1_json_payload())


_PDF_USAGE_FILENAME_ASCII = "EW_usage_guide_v1.pdf"
...
def usage_guide_v1_pdf(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'{disp}; filename="{_PDF_USAGE_FILENAME_ASCII}"',
            "X-Content-Type-Options": "nosniff",
        },
    )


@app.post("/docs/ew-usage-guide-v1/email", response_model=None)
def post_usage_guide_email(request: Request, to: str = Form("")) -> RedirectResponse:
    """登录用户提交邮箱，发送第一版使用守则 PDF 附件（需配置 EW_SMTP_*）。"""
    if not read_session(request):
        return RedirectResponse(
            url="/login?" + urlencode({"next": "/#ew-doc-email"}),
            status_code=303,
        )
    if not ew_smtp_configured():
        return RedirectResponse(
            url="/?" + urlencode({"usage_email_err": "发信未配置：请设置 EW_SMTP_HOST、EW_SMTP_FROM（或 EW_SMTP_USER）等"}),
            status_code=303,
        )
    addr = (to or "").strip()
    try:
        send_usage_guide_pdf_email(addr)
    except ImportError as e:
        return RedirectResponse(
            url="/?"
            + urlencode({"usage_email_err": f"缺少依赖：{e!s}"}),
            status_code=303,
        )
    except ValueError as e:
        return RedirectResponse(
            url="/?" + urlencode({"usage_email_err": str(e)}),
            status_code=303,
        )
    except Exception as e:
        msg = str(e).replace("\n", " ").strip()[:240] or "发送失败"
        return RedirectResponse(
            url="/?" + urlencode({"usage_email_err": msg}),
            status_code=303,
        )
    return RedirectResponse(url="/?usage_email=ok", status_code=303)
