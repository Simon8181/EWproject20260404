...
CREATE TABLE IF NOT EXISTS ew_orders (
    ...
    cargo_density_pcf DOUBLE PRECISION,
    freight_class_nmfc DOUBLE PRECISION,
...
COMMENT ON COLUMN ew_orders.cargo_density_pcf IS '货物密度 lb/ft³（L 重量 ÷ 体积 ft³；体积来自 N 列 m³ 或 M 列三边，格式化数据计算）。';
COMMENT ON COLUMN ew_orders.freight_class_nmfc IS 'NMFC 货运等级（密度法估算，与 cargo_density_pcf 同批写入；正式评级以 NMFTA/承运人为准）。';
